...
function initAddForm(): void {
  categoryForm.value = {
    ...
    nameJa: "",
    nameZh: "",
    sortOrder: props.categoriesCount + 1,
    imageUrl: "",
    iconType: "preset",
    presetIcon: "tag",
  };
...
function initEditForm(category: Category): void {
...
async function saveCategory(): Promise<void> {
  ...
  try {
    ...
    if (props.category) {
      await apiClient.patch(`/api/v1/categories/${props.category.id}`, payload);
    } else {
