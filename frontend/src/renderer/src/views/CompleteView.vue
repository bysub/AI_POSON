...
// 결제 정보 (route query에서 가져옴)
const paymentType = computed(() => route.query.type as string);
const approvalNumber = computed(() => route.query.approvalNumber as string);
const changeAmount = computed(() => {
  const change = route.query.change as string;
  return change ? parseInt(change, 10) : 0;
});

// 주문 번호 (서버에서 받은 것 또는 임시 생성)
const orderNumber = computed(() => {
  const serverOrderNumber = route.query.orderNumber as string;
    ...
    <footer class="mt-auto px-6 pb-8">
      <div class="flex flex-col gap-4">
        ...
        <p class="text-center text-xs font-medium text-gray-400">
          {{ countdown }}{{ t("complete.autoReset") }}
        </p>
      </div>
    </footer>
  </div>
